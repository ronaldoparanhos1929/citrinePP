...
export default function ProductPage() {
  ...
  useEffect(() => {
    const fetchProduct = async () => {
      let p: any = await getProduct(id);
      if (!p) {
        // Mock fallback for preview if not in db
        p = {
          id, name: 'Produto Exemplo Personalizado', description: 'Uma descrição detalhada do produto, informando o material, as opções de personalização e dicas de cuidado.',
          priceType: 'fixed', price: 45.0,
          images: ['https://picsum.photos/seed/p1/600/600', 'https://picsum.photos/seed/p2/600/600', 'https://picsum.photos/seed/p3/600/600'], 
          rating: 4.9, 
          variations: ['P', 'M', 'G', 'GG'],
          isLaunch: false
        };
  ...
  const currentPrice = product.priceType === 'promotional' ? product.promotionalPrice : product.price;
        ...
        <div className="space-y-4">
          <div className="relative aspect-square bg-gray-100 rounded-2xl overflow-hidden group">
            <img 
              src={mainImage} 
              alt={product.name} 
              className="object-cover w-full h-full transition-transform duration-500 group-hover:scale-150 cursor-zoom-in origin-center" 
            />
          </div>
